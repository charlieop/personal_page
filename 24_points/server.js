...
io.on("connection", (socket) => {
  ...
  socket
    .to(socket.request.session.player.room)
    .emit("new player", JSON.stringify(socket.request.session.player));
  ...
  socket.on("start", () => {
    ...
    if (
      room.host != socket.request.session.player.id ||
    ...
    io.to(roomID).emit(
      "game start",
      JSON.stringify({ numbers: numbers, round: room.round })
    );
  });
  ...
  socket.on("win", (round) => {
    ...
    if (room.players[winner].score >= 100) {
      ...
      io.to(socket.request.session.player.room).emit(
        "game end",
  ...
  socket.on("disconnecting", (reason) => {
    ...
    io.to(socket.request.session.player.room).emit(
      "player left",
      JSON.stringify(socket.request.session.player)
    );
